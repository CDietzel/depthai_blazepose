...
def rr2img(rrn_x, rrn_y):
    X = sqn_rr_center_x + sqn_rr_size * (
        (rrn_x - 0.5) * cos_rot + (0.5 - rrn_y) * sin_rot
    )
    Y = sqn_rr_center_y + sqn_rr_size * (
        (rrn_y - 0.5) * cos_rot + (rrn_x - 0.5) * sin_rot
    )
    return X, Y
...
while True:
    ...
    if lm_score > 0.7:
        ...
        if xyz_ref:
            ...
            zone_size = max(int(sqn_rr_size * 1152 / 45), 8)
            xyz_ref_x = filter_x.apply(sqn_xyz_ref_x * 1152 - zone_size / 2 + 0)
            xyz_ref_y = filter_y.apply(sqn_xyz_ref_y * 1152 - zone_size / 2 - 252)
