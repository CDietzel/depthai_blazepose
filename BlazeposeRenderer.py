...
class BlazeposeRenderer:
    def __init__(self, tracker, show_3d=None, output=None):
        ...
        if self.show_3d == "image":
            ...
            self.vis3d.create_grid(
                [0, tracker.img_h, -z],
                [tracker.img_w, tracker.img_h, -z],
                [tracker.img_w, tracker.img_h, z],
                [0, tracker.img_h, z],
                5,
                2,
        ...
        if output is None:
            ...
            self.output = cv2.VideoWriter(
                output, fourcc, tracker.video_fps, (tracker.img_w, tracker.img_h)
            )
    ...
    def draw_landmarks(self, body):
        ...
        if self.show_landmarks:
            ...
            lines = [
                np.array([body.landmarks[point, :2] for point in line])
        ...
        if self.show_score:
            ...
            cv2.putText(
                self.frame,
                f"Landmark score: {body.lm_score:.2f}",
                (20, h - 60),
        ...
        if self.show_xyz_zone and body.xyz_ref:
            ...
            cv2.rectangle(
                self.frame,
                tuple(body.xyz_zone[0:2]),
                tuple(body.xyz_zone[2:4]),
                (180, 0, 180),
                2,
            )
