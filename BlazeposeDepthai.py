...
class BlazeposeDepthai:
    ...
    def __init__(
        ...
        if self.smoothing:
            ...
            self.filter_landmarks = mpu.LandmarksSmoothingFilter(
                frequency=self.video_fps, min_cutoff=0.05, beta=80, derivate_cutoff=1
            )
            ...
            self.filter_landmarks_aux = mpu.LandmarksSmoothingFilter(
                frequency=self.video_fps, min_cutoff=0.01, beta=10, derivate_cutoff=1
            )
        ...
        if self.input_type == "rgb":
            ...
            if self.xyz:
                self.q_spatial_data = self.device.getOutputQueue(
                    name="spatial_data_out", maxSize=1, blocking=False
                )
    ...
    def is_visible(self, body, lm_id):
        if (
            body.visibility[lm_id] > self.visibility_threshold
            and 0 <= body.landmarks[lm_id][0] < self.img_w
            and 0 <= body.landmarks[lm_id][1] < self.img_h
        ):
            return True
        else:
            return False
    ...
    def query_body_xyz(self, body):
        ...
        # - None otherwise
        if self.is_visible(body, mpu.KEYPOINT_DICT["right_hip"]) and self.is_visible(
            body, mpu.KEYPOINT_DICT["left_hip"]
        ):
            ...
            body, mpu.KEYPOINT_DICT["right_shoulder"]
        ) and self.is_visible(body, mpu.KEYPOINT_DICT["left_shoulder"]):
        ...
        roi_center = dai.Point2f(
            int(body.xyz_ref_coords_pixel[0] - zone_size / 2 + self.crop_w),
            int(body.xyz_ref_coords_pixel[1] - zone_size / 2),
        )
        ...
        body.xyz = np.array(
            [sd.spatialCoordinates.x, sd.spatialCoordinates.y, sd.spatialCoordinates.z]
        )
        if self.smoothing:
    ...
    def lm_postprocess(self, body, inference):
        ...
        if body.lm_score > self.lm_score_thresh:
            ...
            dst = np.array(
                [(x, y) for x, y in body.rect_points[1:]], dtype=np.float32
    ...
    def next_frame(self):
        ...
        if self.force_detection or not self.use_previous_landmarks:
            ...
            if self.input_type != "rgb" and (
                self.force_detection or not self.use_previous_landmarks
            ):
        ...
        if body:
            ...
            frame_nn = frame_nn / 255.0
            nn_data = dai.NNData()
            nn_data.setLayer(
                "input_1",
                to_planar(frame_nn, (self.lm_input_length, self.lm_input_length)),
            )
            lm_rtrip_time = now()
    ...
    def exit(self):
        ...
        if self.stats:
            ...
            if self.input_type != "rgb" and self.nb_pd_inferences != 0:
                print(
                    f"Pose detection round trip   : {self.glob_pd_rtrip_time/self.nb_pd_inferences*1000:.1f} ms"
                )
            if self.nb_lm_inferences != 0:
                print(
                    f"Landmark round trip         : {self.glob_lm_rtrip_time/self.nb_lm_inferences*1000:.1f} ms"
                )
